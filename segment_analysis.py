...
def build_pivot_table(
    ...
    if (
        index_col not in df.columns
        or columns_col not in df.columns
        or value_col not in df.columns
    ):
        raise ValueError(
            "Required columns are missing for pivot table creation"
        )
...
def write_segment_insights(
    ...
    lines.append(
        "\nPivot table by segment and product:"
    )
...
if __name__ == "__main__":
    ...
    report = write_segment_insights(df)
